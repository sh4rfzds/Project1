...
# --- Configuration ---
CALC_STRESS_DIR = CALC_DIRS["stress"]
STRESS_MOLS_DIR = os.path.join(PROJECT_DIR, "stress_test_molecules")

SP_TEMPLATE = os.path.join(TEMPLATES_DIR, "sp2template.inp")
LOG_FILE = os.path.join(CALC_STRESS_DIR, "stress_workflow.log")
...
def prepare_sp(base_mol, doped_mol):
    work_dir = os.path.join(CALC_STRESS_DIR, base_mol, doped_mol, "sp")
    os.makedirs(work_dir, exist_ok=True)
    
    xyz_src = os.path.join(STRESS_MOLS_DIR, base_mol, f"{doped_mol}.xyz")
...
def run_phase(phase_name, suffix, prepare_func, base_mol):
    files = sorted([f for f in os.listdir(os.path.join(STRESS_MOLS_DIR, base_mol)) if f.endswith(".xyz")], key=natural_keys)
    job_queue = [get_short_name(f) for f in files]
    
    pending_jobs = []
    for doped_mol in job_queue:
        out_path = os.path.join(CALC_STRESS_DIR, base_mol, doped_mol, suffix.lower(), f"{doped_mol}_{suffix.lower()}.out")
...
def main():
    if not os.path.exists(STRESS_MOLS_DIR):
        log_message(f"CRITICAL: {STRESS_MOLS_DIR} does not exist.")
        return
    if not os.path.exists(TEMPLATES_DIR):
        log_message(f"CRITICAL: {TEMPLATES_DIR} does not exist. Please add sp2template.inp")
        return

    os.makedirs(CALC_STRESS_DIR, exist_ok=True)
    log_message("=====================================================")
    log_message("--- Stress Test Sp Scheduler Started ---")
    log_message("=====================================================")

    base_mols = sorted([d for d in os.listdir(STRESS_MOLS_DIR) if os.path.isdir(os.path.join(STRESS_MOLS_DIR, d))], key=natural_keys)
    ...
    for base_mol in base_mols:
        ...
        doped_files = sorted([f for f in os.listdir(os.path.join(STRESS_MOLS_DIR, base_mol)) if f.endswith(".xyz")], key=natural_keys)
        pending_analysis = []
        
        for f in doped_files:
            doped_mol = get_short_name(f)
            out_path = os.path.join(CALC_STRESS_DIR, base_mol, doped_mol, "sp", f"{doped_mol}_sp.out")
            if check_if_successful(out_path):
                sp_dir = os.path.join(CALC_STRESS_DIR, base_mol, doped_mol, "sp")
