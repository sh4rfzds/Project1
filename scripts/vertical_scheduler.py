...
LOG_FILE = os.path.join(CALC_VERT_DIR, "vertical_workflow.log")
...
def prepare_spguess(base_mol, doped_mol):
    work_dir = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, "spguess")
...
def prepare_sp(base_mol, doped_mol):
    spguess_dir = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, "spguess")
    work_dir = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, "sp")
    os.makedirs(work_dir, exist_ok=True)
...
def run_phase(phase_name, suffix, prepare_func, base_mol):
    ...
    for doped_mol in job_queue:
        out_path = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, suffix.lower(), f"{doped_mol}_{suffix.lower()}.out")
...
def main():
    ...
    os.makedirs(CALC_VERT_DIR, exist_ok=True)
    log_message("=====================================================")
    log_message("--- Doped Molecule Vertical Sp Scheduler Started ---")
    log_message("=====================================================")
    ...
    for base_mol in base_mols:
        ...
        for f in doped_files:
            ...
            out_path = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, "sp", f"{doped_mol}_sp.out")
            if check_if_successful(out_path):
                sp_dir = os.path.join(CALC_VERT_DIR, base_mol, doped_mol, "sp")
