...
# --- Configuration ---
CALC_ADDITIVITY_DIR = CALC_DIRS["additivity"]
DOPED_MOLECULES_DIR = os.path.join(PROJECT_DIR, "double_doped_molecules")

SPGUESS_TEMPLATE = os.path.join(TEMPLATES_DIR, "spguesstemplate.inp")
SP_TEMPLATE = os.path.join(TEMPLATES_DIR, "sptemplate.inp")
LOG_FILE = os.path.join(CALC_ADDITIVITY_DIR, "additivity_workflow.log")
...
def prepare_spguess(base_mol, doped_mol):
    work_dir = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "spguess")
    os.makedirs(work_dir, exist_ok=True)
    
    xyz_src = os.path.join(DOPED_MOLECULES_DIR, base_mol, f"{doped_mol}.xyz")
    ...
    if not os.path.exists(SPGUESS_TEMPLATE): return None, f"Missing {SPGUESS_TEMPLATE}"
    with open(SPGUESS_TEMPLATE, 'r') as f: template_content = f.read()

    coord_block = f"* xyz {CHARGE} {MULT}\n{coords}*\n"
    final_inp = f"{template_content.strip()}\n\n{coord_block}"
    
    inp_path = os.path.join(work_dir, f"{doped_mol}_spguess.inp")
    with open(inp_path, "w") as f: f.write(final_inp)
    return inp_path, "Ready"

def prepare_sp(base_mol, doped_mol):
    spguess_dir = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "spguess")
    work_dir = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "sp")
    os.makedirs(work_dir, exist_ok=True)
    
    spguess_out = os.path.join(spguess_dir, f"{doped_mol}_spguess.out")
    if not check_if_successful(spguess_out): return None, "SPGUESS not successfully completed"
    ...
    gbw_src = os.path.join(spguess_dir, f"{doped_mol}_spguess.gbw")
    gbw_dest = os.path.join(work_dir, f"{doped_mol}_guess.gbw")
    if os.path.exists(gbw_src):
        shutil.copy2(gbw_src, gbw_dest)
    else:
        return None, "Missing SPGUESS .gbw file"

    if not os.path.exists(SP_TEMPLATE): return None, f"Missing {SP_TEMPLATE}"
    with open(SP_TEMPLATE, 'r') as f: template_content = f.read()

    moinp_block = f'%moinp "{doped_mol}_guess.gbw"'
    coord_block = f"* xyz {CHARGE} {MULT}\n{coords}*\n"
    final_inp = f"{template_content.strip()}\n\n{moinp_block}\n\n{coord_block}"
    
    inp_path = os.path.join(work_dir, f"{doped_mol}_sp.inp")
    with open(inp_path, "w") as f: f.write(final_inp)
    return inp_path, "Ready"

# --- Analysis & Plotting Functions ---

def run_plotting(base_mol, doped_mol):
    work_dir = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "sp")
    gbw = f"{doped_mol}_sp.gbw"
    out = f"{doped_mol}_sp.out"
    ...
    log_message(f"Starting: {doped_mol} (PLOT)")
    ...
    try:
        ...
        log_message(f"Job Completed: {doped_mol} (PLOT) | SUCCESS")
    except Exception as e:
        log_message(f"Plotting Failed for {doped_mol}: {e}")
...
def run_phase(phase_name, suffix, prepare_func, base_mol):
    target_dir = os.path.join(DOPED_MOLECULES_DIR, base_mol)
    files = sorted([f for f in os.listdir(target_dir) if f.endswith(".xyz")], key=natural_keys)
    job_queue = [get_short_name(f) for f in files]
    
    pending_jobs = []
    for doped_mol in job_queue:
        out_path = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, suffix.lower(), f"{doped_mol}_{suffix.lower()}.out")
        if not check_if_successful(out_path):
            pending_jobs.append(doped_mol)
...
def main():
    ...
    if not os.path.exists(TEMPLATES_DIR):
        log_message(f"CRITICAL: {TEMPLATES_DIR} does not exist. Please add spguesstemplate.inp and sptemplate.inp")
        return

    os.makedirs(CALC_ADDITIVITY_DIR, exist_ok=True)
    log_message("=====================================================")
    log_message("--- Additivity Vertical Sp Scheduler Started ---")
    log_message("=====================================================")
    ...
    for base_mol in base_mols:
        ...
        run_phase("SPGUESS", "SPGUESS", prepare_spguess, base_mol)
        if shutdown_requested: break
        ...
        target_dir = os.path.join(DOPED_MOLECULES_DIR, base_mol)
        doped_files = sorted([f for f in os.listdir(target_dir) if f.endswith(".xyz")], key=natural_keys)
        pending_analysis = []
        
        for f in doped_files:
            doped_mol = get_short_name(f)
            out_path = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "sp", f"{doped_mol}_sp.out")
            if check_if_successful(out_path):
                sp_dir = os.path.join(CALC_ADDITIVITY_DIR, base_mol, doped_mol, "sp")
                cube_count = len([c for c in os.listdir(sp_dir) if c.endswith(".cube")])
                if cube_count < 4:
                    pending_analysis.append(doped_mol)
        ...
        if not pending_analysis:
            ...
            for doped_mol in pending_analysis:
                if shutdown_requested: break
                run_plotting(base_mol, doped_mol)
