...
# --- Configuration ---
BASE_MOLECULES_DIR = BASE_MOLS_DIR
...
LOG_FILE = os.path.join(CALC_BASE_DIR, "base_workflow.log")
...
def prepare_opt(mol_name):
    work_dir = os.path.join(CALC_BASE_DIR, mol_name, "opt")
    os.makedirs(work_dir, exist_ok=True)
    
    xyz_src = os.path.join(BASE_MOLECULES_DIR, f"{mol_name}.xyz")
...
def prepare_sp(mol_name):
    opt_dir = os.path.join(CALC_BASE_DIR, mol_name, "opt")
    work_dir = os.path.join(CALC_BASE_DIR, mol_name, "sp")
    os.makedirs(work_dir, exist_ok=True)
...
def run_phase(phase_name, suffix, prepare_func):
    files = sorted([f for f in os.listdir(BASE_MOLECULES_DIR) if f.endswith(".xyz")])
    job_queue = [get_short_name(f) for f in files]
    
    pending_jobs = []
    for job in job_queue:
        out_path = os.path.join(CALC_BASE_DIR, job, suffix.lower(), f"{job}_{suffix.lower()}.out")
        if not check_if_successful(out_path):
            pending_jobs.append(job)

    if not pending_jobs:
        log_message(f"--- Phase {phase_name}: Queue (0 jobs) - All completed ---")
        return

    log_message(f"--- Phase {phase_name}: Queue ({len(pending_jobs)} jobs) ---")
    ...
    while (pending_jobs or running_jobs) and not shutdown_requested:
        ...
        while len(running_jobs) < MAX_CONCURRENT_JOBS and pending_jobs:
            ...
            inp_path, status = prepare_func(next_name)
...
def main():
    if not os.path.exists(BASE_MOLECULES_DIR):
        log_message(f"CRITICAL: {BASE_MOLECULES_DIR} does not exist.")
        return
    if not os.path.exists(TEMPLATES_DIR):
        log_message(f"CRITICAL: {TEMPLATES_DIR} does not exist. Please add opttemplate.inp and sptemplate.inp")
        return

    os.makedirs(CALC_BASE_DIR, exist_ok=True)
    log_message("=====================================================")
    log_message("--- Base Molecule Opt and Sp Scheduler Started ---")
    log_message("=====================================================")

    run_phase("OPT", "OPT", prepare_opt)
    if shutdown_requested: return

    run_phase("SP", "SP", prepare_sp)
    if shutdown_requested: return
    ...
    for f in files:
        ...
        out_path = os.path.join(CALC_BASE_DIR, mol, "sp", f"{mol}_sp.out")
        if check_if_successful(out_path):
            sp_dir = os.path.join(CALC_BASE_DIR, mol, "sp")
            cube_count = len([c for c in os.listdir(sp_dir) if c.endswith(".cube")])
            if cube_count < 4:
                pending_plots.append(mol)
                
    if not pending_plots:
        log_message("--- Phase PLOT: Queue (0 jobs) - All completed ---")
    else:
        log_message(f"--- Phase PLOT: Queue ({len(pending_plots)} jobs) ---")
        for mol in pending_plots:
